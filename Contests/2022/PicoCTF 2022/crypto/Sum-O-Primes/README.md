# Sum-O-Primes

## Challenge

Lại là `RSA` và công việc vẫn như cũ là tìm cách `factor N`. Lần này có dễ dàng hơn khi có thêm giả thiết `x = p + q` và "hiển nhiên" ta đã có `n = p * q`, vậy chỉ việc giải phương trình bậc 2 là xong!!!

## Solution
```py
x = 
n = 
c = 
e = 0x10001

from gmpy2 import *
from Crypto.Util.number import *

def quadratic_eq(a, b, c):
    delta = b**2 - 4*a*c
    return (-b + iroot(delta, 2)[0])//(2*a)

p = quadratic_eq(1, -x, n)
q = n//p

n_phi_Euler = (p - 1) * (q - 1)
d = pow(e, -1, n_phi_Euler)

flag = long_to_bytes(pow(c, d, n))

print(flag)
```

## Flag
**`picoCTF{92fe3557}`**