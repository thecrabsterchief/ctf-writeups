...
s = remote("103.245.250.31", 30620)
s.recvuntil(b'WELCOME TO RANDOM MACHINE!\nHere is the first 665 random numbers.\n')
n = [int(x) for x in s.recvline().decode().split(' ')]

mtb = MT19937Recover()
r = mtb.go(n)
s.recvuntil(b"Now it's your turn to guess the last random number:\n")
s.sendline(str(r.getrandbits(32)).encode())
s.recvuntil(b'Here is your flag.\n')

flag = s.recvline()[:-1].decode()
